...
- **LLM provider seam** (`cua/llm.py`). Brief was written aroundAnthropic; live runs here were recorded against OpenAI (that accountis funded). Both providers are first-class; every tool-translationtest exercises both shapes. `gpt-5` rejects the legacy `max_tokens`parameter, so the OpenAI client sends `max_completion_tokens` and`response_format={"type":"json_object"}` for the JSON proposal call.`demo_agent_invocation.py` shares this same client — no separateconstruction, no model-specific workaround — so it runs end-to-endunder the default model.
...
- **Policy gate at the action boundary.** `cua/policy.py::PolicyGate`is the single choke point. `policy.yaml` declares an allowlist ofdomains/routes and `require_confirmation_patterns` (verbs like`submit`, `delete`, `transfer`); discovery and replay both call`PolicyGate.evaluate(action)` before Playwright touches the page, sonothing outside the allowlist — or a destructive verb without anapproved confirmation — executes. Evidence:`evidence/discovery_open_subaccount_20260818T173007Z/` ends with thesub-account form filled but `Submit` never attempted; the model wastold to treat the completed form as success precisely because thegate would refuse the submit.
- **Sensitivity-driven redaction at the write boundary.** `Sensitivity = Literal["public", "internal", "pii", "secret"]` (`cua/schema.py:19`).Every write under `cua/evidence.py` (discovery and replay) redacts bydeclared ParamSpec sensitivity first (`pii`/`secret` become shape-only,e.g. `<redacted:string:5>`), then applies a regex fallback from`policy.yaml::redaction_rules` to free-text `message`/`error` fields.`cua/catalog.py` strips `example` from any `sensitivity=secret`ParamSpec before publishing the tool schema (`catalog.py:160`).
- **A parallel writer had the same control, unwired.**`demo_agent_invocation.py` doesn't go through `cua/evidence.py` — itkeeps its own transcript. It defined `_redact_arguments_string` and`_redact_anthropic_content` (both unit-tested) to scrub the raw`tool_calls[].arguments`/`content` blob before writing `turns[]`, butnever called either. Caught by actually running the demo on thecurrent default model: the committed transcript held `demo123` inplaintext. Fixed by wiring both helpers into `_shared_loop`; a rerunconfirms zero occurrences of the raw value. Clean evidence:`evidence/agent_catalog_invocation_20260818T175556Z/transcript.json`(item 7 in `evidence/EVIDENCE.md`; see its Superseded section for thefull chain).
...
- **Tests cover schema, registry, error taxonomy, and policy gate —not end-to-end Playwright in CI.** 147 unit tests exercise the schema(validation, version-bump classification), registry (immutability,overlay compose), locator ladder (drift telemetry, CSS-primaryrejection), error taxonomy (`ReplayResult` variants, exit codes),policy gate (allowlist, redaction), catalog (tool translation,secret-example stripping), escalate (state machine, holderassertion, resume-token gating), and discovery (unchanged-observationguard). End-to-end Playwright replays against a live app are provenby the committed evidence pack, not a CI job — the live suite isexpensive and browser-flaky, and would gate PRs on unrelatedChromium updates.
